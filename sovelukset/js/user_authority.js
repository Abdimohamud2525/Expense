$(document).ready(function () {
  ...
  $("#userForm").submit(function (event) {
    ...
    console.log("User ID before form submit:", user_id) // Lisää tämä
    ...
    console.log("Toiminnot ennen lomakkeen lähettämistä:", actions)

    // // Tarkistetaan, että user_id ja actions eivät ole tyhjiä
    if (!user_id || actions.length === 0) {
      console.log("Pakolliset kentät puuttuvat")
      return
    }
    let sendingData = {
      user_id: user_id,
      ...
      success: function (data) {
        ...
        if (status) {
          if (response.length >= 1) {
            response.forEach((res) => {
              $(
                'input[type="checkbox"][name="role_authority[]"]'[
                  (value = "${users.role}")
                ]
              ).prop("checked", true)
              $(
              ...
              $(
                'input[type="checkbox"][name="system_action[]"]'[
                  (value = "${users.action_id}")
                ]
              ).prop("checked", true)
            })
          } else {
            $('input[type="checkbox"][name="role_authority[]"]').prop(
              "checked",
              false
            )
          }
            ...
            if (res.action_name !== system_actions) {
              html += `
                <div class="system_action">
                  <label class="control-label input-label">
                    <input type="checkbox" name="system_action[]" role="${res.role}" value="${res.action_id}" category_id="${res.category_id}" link_id="${res.link_id}">
                    ${res.action_name}
                  </label>
                </div>`
